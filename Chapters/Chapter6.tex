...
While the "ACCEPT" case has been fully implemented, the "REJECT" case has not been implemented at all. Figure \ref{monitor:reject} shows that in the "REJECT" case, all that is occuring is the memory is being erased before the local secure mode value is sent out of the monitor.\\ %% XXX -- ASK PAUL ABOUT THE SECURE MODE REJECT ROUTINE ONCE THE MONITOR HAS RECIEVED THE REJECT COMMAND %%
